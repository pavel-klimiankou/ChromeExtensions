...
var core = FBCaseViewer;

core.onCaseLoad(function () {
	var cases = core.getCases();

	cases.forEach(function (issue) {
		var header = core.getCaseHeader(issue),
			body = core.getCaseBody(issue),
			original = body.cloneNode(true),
			elementId = "i" + Math.random().toString().replace(".", "");

			header.insertAdjacentHTML("beforeend", 
				'<input type="checkbox" checked="true" id="{0}"><label for="{0}">Prettify</label>'.replace(/\{0\}/g, elementId)
			);

			header.querySelector("#" + elementId).addEventListener("change", function () {
				var temp = body;
				body.parentNode.replaceChild(original, body);
		...
		formatIssue(core.getCaseBody(cases[0]));
	});
});
